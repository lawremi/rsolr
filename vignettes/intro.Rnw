...
head(sr)
@ 
The \texttt{head()} method returns virtually instantaneously, because the
...
is easy to add one and indicate as such to \texttt{deriveSolrSchema()}.
...
\item \texttt{var}, \texttt{sd} (\emph{not yet working with rsolr, as Solr is evolving}),
\item \texttt{sum},
